# e-commerce
# Kigali Eats Fresh Store

## Student Information

- Name: Clementine Iradukunda
- Registration Number: 23771/2024
-Course: E-Commerce And Web Application (EWA408510)
-Lecturer: Eric Maniraguha
-Academic Year: 2025–2026 | Semester II
Submission Date: June 08, 2026

## Project Title

  Kigali Eats Fresh Store E-Commerce Website

## Platform Used

Wix

## Project Description

Kigali Eats Fresh Store is an online food store developed using Wix. The website allows customers to browse food products, view prices, learn about the business, and interact with a shopping cart.


## Features Implemented

- Homepage with welcome message
- Product page with 10 products
- About page
- Contact page
- Shopping cart functionality
- Mobile-friendly design

## Screenshots

### Homepage

![Homepage](image/home.JPG)

### Product Page

![Products](images/product.jpg)

### Contact Page

![Contact](images/contact.jpg)

## About page

![About](images/about.jpg)

## Cart page

![cart](images/cart.jpg)

## Phone page

![phone](images/phone.jpg)



## Challenges Faced

- Choosing an appropriate website template
- Organizing products correctly
- Customizing website colors and layout
-to be familia with it tool and features

## Lessons Learned

- How to build an e-commerce website using Wix
- How to customize templates
- How to publish a website
- How to use GitHub for project documentation


## Live Website Link

https://clementineiradukun7.wixsite.com/my-site-4


## GitHub Repository Link

https://github.com/clementine-Iradukunda/e-commerce
